...
export const PlayerSidebar = ({
  ...
  const poorestPlayer = players.reduce((poorest, p) => 
    p.money < poorest.money ? p : poorest
  );
      ...
      <div className="overflow-y-auto max-h-[500px]">
        <AnimatePresence>
          {sortedPlayers.map((player, index) => {
            ...
            const moneyPercentage = (player.money / totalMoney) * 100;
                          ...
                          <div className="mt-2 bg-danger-500/10 border border-danger-500/20 rounded-lg p-2">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm text-danger-300">
                                In Jail for {player.jailTurns || 0} more turns
                              </span>
                            </div>
                          </div>
                        )}
      ...
      <div className="flex flex-wrap gap-2">
        {players.map(player => (
          <div
            ...
            title={`${player.name}: ₹${player.money / 1000}k`}
          >
            <div
            ...
            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
              <div className="glass-panel px-3 py-2 rounded-lg whitespace-nowrap text-sm">
                ...
                <div className="text-xs text-text-muted">₹{(player.money / 1000).toFixed(1)}k</div>
              </div>
            </div>
          </div>
        ))}
