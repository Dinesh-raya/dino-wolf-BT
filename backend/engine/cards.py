...
class CardEngine:
    def __init__(self):
        ...
        random.shuffle(self.treasury_cards)
        random.shuffle(self.surprise_cards)
        
    def draw_treasury(self, game_state: GameState, player_id: str) -> Dict[str, Any]:
        card = self.treasury_cards.pop(0)
        self.treasury_cards.append(card)
        self.execute_card(game_state, player_id, card)
        return card
        
    def draw_surprise(self, game_state: GameState, player_id: str) -> Dict[str, Any]:
        card = self.surprise_cards.pop(0)
        self.surprise_cards.append(card)
        self.execute_card(game_state, player_id, card)
        return card
    ...
    def execute_card(self, game_state: GameState, player_id: str, card: Dict[str, Any]):
        ...
        game_state.history_log.append(f"{player.name} drew a card: {card['text']}")
        ...
        if action == "add_money":
            ...
            if target < current and target != GameRules.JAIL_TILE:
                ...
                game_state.history_log.append(f"{player.name} passed GO and collected ₹{GameRules.GO_REWARD}")
