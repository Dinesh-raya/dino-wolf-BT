...
def declare_bankruptcy(game_state: GameState, debtor_id: str, creditor_id: Optional[str] = None):
    ...
    if creditor_id:
        ...
        game_state.history_log.append(f"{debtor.name} went bankrupt and transferred assets to {creditor.name}")
    else:
        # Return to bank
        for prop_id in debtor.properties_owned:
            prop_state = game_state.properties[prop_id]
        ...
        game_state.history_log.append(f"{debtor.name} went bankrupt. Assets returned to the bank.")
    ...
    if len(active_players) == 1:
        game_state.room.status = "finished"
        game_state.history_log.append(f"Game Over! {active_players[0].name} wins!")
